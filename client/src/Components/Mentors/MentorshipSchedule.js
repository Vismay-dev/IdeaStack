  ...
  useEffect(() => {
    ...
    if (props.workshops && props.workshops.length !== 0) {
      ...
      for (let i = 0; i < props.workshops[0].timeline.length; i++) {
        if (
          currentTime < props.workshops[0].timeline[i] &&
          (i == 0 || currentTime > props.workshops[0].timeline[i - 1])
        ) {
          currSessionNum = i;
        }
      }
        ...
        >
          <h1
            class={`sm:text-3xl text-2xl  ${
              num === 4 ? "md:top-5 top-5" : "sm:top-4 top-5"
          ...
          <h1
            ...
            }  text-center font-semibold relative text-gray-200`}
          >
            {workshop &&
              workshop.mentors &&
