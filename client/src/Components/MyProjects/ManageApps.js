...
<h2 class = {`text-center font-bold sm:text-5xl ${user&&project&&user._id!==project.admin.id?'py-4':''} text-4xl sm:px-0 px-8 text-gray-800 top-1 relative sm:mt-10 mt-11 sm:-mb-2.5 -mb-4`}><svg xmlns="http://www.w3.org/2000/svg" class="md:h-10 md:w-10 h-8 w-8 mr-1 inline relative bottom-[3px]" viewBox="0 0 20 20" fill="currentColor">
  <path fill-rule="evenodd" d="M6.672 1.911a1 1 0 10-1.932.518l.259.966a1 1 0 001.932-.518l-.26-.966zM2.429 4.74a1 1 0 10-.517 1.932l.966.259a1 1 0 00.517-1.932l-.966-.26zm8.814-.569a1 1 0 00-1.415-1.414l-.707.707a1 1 0 101.415 1.415l.707-.708zm-7.071 7.072l.707-.707A1 1 0 003.465 9.12l-.708.707a1 1 0 001.415 1.415zm3.2-5.171a1 1 0 00-1.3 1.3l4 10a1 1 0 001.823.075l1.38-2.759 3.018 3.02a1 1 0 001.414-1.415l-3.019-3.02 2.76-1.379a1 1 0 00-.076-1.822l-10-4z" clip-rule="evenodd" />
</svg> <p class = 'inline'>Manage Applications</p></h2>
    ...
    
    {
      ...
      <><h1 class = 'sm:text-4xl text-3xl px-6 text-center sm:mt-[67px] mt-[64px]'><span class = {!acceptingApp?'text-blue-700':'text-red-500'}>{acceptingApp?'Pause':'Accept'}</span> Core-Team Applications</h1>
    ...
    <label for="toggleB" class="flex items-center cursor-pointer">
      ...
      <div class="relative mt-6 -mb-1">
        ...
        {/* <!-- line --> */}
        <div id = {styles.bgrnd} class="block bg-gray-600 sm:w-14 w-12 sm:h-8 h-7 rounded-full"></div>
        {/* <!-- dot --> */}
        <div id= {styles.dot} className="dot absolute left-1 top-1 bg-white sm:w-6 sm:h-6 w-5 h-5 rounded-full transition"></div>
      </div>
...
<div class={`rounded-lg shadow-lg bg-gradient-to-r  border-[1px] border-blue-600  from-blue-50 to-indigo-200 overflow-hidden mb-0`}>
  ...
  <p className="text-center top-4 text-xl font-semibold relative px-10">Applications Accepted: </p><br/>
    ...
    <p class={`text-sm relative ${!latestAccepted?'top-3 ':' top-1.5'} text-center font-light text-gray-600 xl:px-[80px] lg:px-[70px] md:px-[60px] px-[55px]  `}>Latest Accepted Team Member: {latestAccepted?<><span class = 'text-indigo-500 font-semibold'>{latestAccepted.name}</span> ({latestAccepted.date?Date(latestAccepted.date).toString().substring(0,15):''})</>:'--'} </p>    
...
<div class={`rounded-lg shadow-lg bg-gradient-to-r  relative bottom-[0.7px] border-[1px] border-blue-600  from-blue-50 to-indigo-200 overflow-hidden mb-0`}>
  ...
  <p class="text-sm relative text-center font-light bottom-0.5 text-gray-600 xl:px-[80px] lg:px-[50px] lg:mb-1.5  xl:mt-0 ">Have a look at join requests for <span class = 'text-indigo-500 font-semibold'> IdeaStack</span>!</p>     
