    ...
    return (
        ...
        <div class="w-full mx-auto pt-10 md:pb-0 pb-1   md:px-14 sm:px-8 px-4 lg:-mb-[190px] md:-mb-[180px] -mb-[220px] relative lg:max-w-7xl ">
            <h2 style = {{'backgroundImage':'url(https://upload.wikimedia.org/wikipedia/commons/8/81/Maxresdefault_%281%29.jpg)'}} class = 'text-center bg-no-repeat z-40 bg-gradient-to-br from-cyan-400 to-cyan-600   text-gray-100 top-1  bg-right w-full ring-2 ring-offset-2 ring-offset-cyan-500 border-2 border-blue-700 ring-blue-700 rounded-sm shadow-md mb-[74px] relative'>
              ...
              <svg xmlns="http://www.w3.org/2000/svg" class="md:h-14 sm:h-10 h-8 bottom-[3px] mr-0.5 -ml-1.5 relative md:w-14 sm:w-10 w-8 sm:inline hidden" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
  <path d="M12 14l9-5-9-5-9 5 9 5z" />
  <path d="M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z" />
<path stroke-linecap="round" stroke-linejoin="round" d="M12 14l9-5-9-5-9 5 9 5zm0 0l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14zm-4 6v-7.5l4-2.222" />
</svg> Seek Mentorship</p>
              <p class = 'md:text-2xl text-xl  mt-0.5 left-1.5 relative font-semibold'>Find the Right Expert</p>
              </div>
              </h2>
    ...
    <div class=" border-2 md:col-span-4 col-span-6 grid sm:grid-cols-6 sm:text-base text-sm grid-cols-12 pointer-events-auto rounded-sm border-gray-300">
        <input type="text" onChange={textChangeHandler} class="px-4 py-2 sm:text-base text-sm sm:col-span-5 col-span-9 shadow-sm border-0" placeholder="Search Expert"/>
        <button onClick={()=>filterByText()} class="items-center sm:col-span-1 col-span-3 hover:shadow-lg hover:bg-blue-700 active:shadow-sm shadow-sm justify-center px-4 border-l bg-blue-600">
            <svg class="w-6 h-6 text-gray-100 text-center mx-auto block" fill="currentColor" xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24">
                <path
            ...
            [...experts].map((expert,i) => (
              <a data-aos={"fade-up"} data-aos-once='true' delay = {`${i%3}00`} key={expert.id} onClick={()=> {setShowExpert(true); setExpertId(expert.id)}} class={`group z-20 pointer-events-auto  rounded-md cursor-pointer`}>
                <div  class="w-full z-20  shadow-md     overflow-hidden ">
                  <img
                    src={expert.pic?expert.pic:'https://images.unsplash.com/photo-1583864697784-a0efc8379f70?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxzZWFyY2h8MTV8fG1hbGV8ZW58MHx8MHx8&w=1000&q=80'}
                    alt="expert"
                    class="w-[270px] mx-auto shadow-xl   rounded-[.25rem]  h-[270px]   object-center object-cover group-hover:opacity-75"
                  />
                </div>
                <h3 class="mt-4 text-md sm:text-left lg:right-0 relative md:-right-14 text-center text-gray-700">{expert.role}</h3>
                <p class="mt-1 text-lg sm:text-left lg:right-0 relative md:-right-14 text-center font-medium text-gray-900">{expert.name}</p>
              </a>
            ))}
          </div>
  }
