...
const MoreDetails = (props) => {
    ...
    if (user && project) {
      for (let i = 0; i < Array(...user.joinRequests).length; i++) {
        for (let k = 0; k < project.joinRequests.length; k++) {
          if (
            JSON.stringify(project.joinRequests[k]) ==
              JSON.stringify(user.joinRequests[i]) &&
            !user.joinRequests[i].isInvite
          ) {
  ...
  const submitHandler = (e) => {
    ...
    let applicationTemp = {
      ...
      id: user._id,
      projID: project._id,
      appStatus: "Not Decided",
    };
    setApplication(applicationTemp);
    axios
      .post(
        process.env.NODE_ENV === "production"
          ? "https://ideastack.herokuapp.com/api/user/createJoinRequest"
          : "http://localhost:4000/api/user/createJoinRequest",
        {
          token: sessionStorage.getItem("token"),
  ...
  return (
    <div
      class="fixed z-[85] inset-0 overflow-y-auto"
    ...
    >
      <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div
          ...
          data-aos={"fade-up"}
          data-aos-once="true"
          class="pr-6 inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:mt-5 sm:align-middle sm:w-10/12 w-11/12"
        >
          {!applying && !appLoading ? (
            <div class="bg-white px-4 pt-4 pb-6 sm:p-6 sm:pb-4">
              ...
              <div
                class={`grid grid-cols-2 xl:h-[510px] h-[1020px] mx-auto relative left-2 sm:mt-10 mt-5 ${
                  isInTeam ? "mb-9" : "mb-6"
                } gap-5`}
              >
                <div class="bg-gradient-to-br xl:col-span-1 col-span-2 xl:pb-4 sm:pb-0 pb-5 from-blue-50 to-indigo-100 rounded-sm shadow-md">
                  {projLoading ? (
                    ...
                    <div
                      class={`w-full sm:px-8 px-4 sm:pt-1 pt-11 py-1 mt-1 mr-4 h-full bg-transparent  `}
                    >
                      <div class="flex items-center sm:justify-left justify-center">
                        <span class="text-sm top-2  relative mx-auto font-light text-gray-600 ">
                          {project ? date : ""}
                        </span>
                      </div>
                      ...
                      <div class="mt-2 sm:text-left text-center">
                        <a
                          href="#"
                          class="sm:text-2xl text-xl relative font-bold text-gray-700 hover:text-gray-600  hover:underline"
                        >
                      ...
                      <>
                        ...
                        <img
                          class="md:mt-8 sm:mt-7 mt-5 md:mb-9 mb-11 border-2 py-5 rounded-sm bg-white  object-contain md:w-72 sm:w-64 w-48 h-40  sm:h-52 shadow-lg mx-auto relative md:h-56"
                          src={project.projPic}
                        ></img>
                      </>
                      ...
                      <div class="flex items-center  md:justify-between justify-center sm:mr-0 mr-2 sm:mb-0 sm:pb-0 pb-3 -mb-4 relative">
                        <div class="md:flex block items-center relative bottom-0.5">
                          <img
                            class="object-cover sm:w-10 sm:h-10 w-6 h-6 mx-2  rounded-full md:block sm:inline  hidden"
                            src={
                              project.admin && project.admin.pic
                                ? project.admin.pic
                          ...
                          <a class="font-bold text-gray-700 cursor-pointer inline sm:text-md text-sm mx-auto text-center relative left-1 ">
                            {project.admin ? project.admin.name : ""} - Project
                            Admin
                          </a>
                          ...
                          <p
                            ...
                            class="text-blue-700 w-fit text-center mx-auto md:inline block hover:underline md:ml-3 sm:ml-[50%] ml-[27%] relative text-sm font-semibold hover:text-blue-800 cursor-pointer"
                          >
                            {" "}
                            ({" "}
                            <svg
                            ...
                            <span class="inline">View Profile</span>)
                          </p>
                        </div>
                      </div>
                    </div>
                ...
                <div class="bg-gradient-to-br sm:px-8 px-2 py-7 xl:col-span-1 col-span-2 text-center from-indigo-100 to-blue-50  rounded-sm shadow-md">
                  {projLoading ? (
                    ...
                    <>
                      <h3 class="mb-3 xl:mt-12 mt-7 sm:text-md text-sm tracking-wide">
                        <strong>Category</strong>:<br class="sm:hidden block" />{" "}
                        {project.category ? project.category : ""}
                      </h3>
                      <h3 class="mb-3 tracking-wide sm:text-md text-sm ">
                        <strong>Maximum Team Capacity</strong>:
                        <br class="sm:hidden block" />{" "}
                        {project.maxCap ? project.maxCap : ""}
                      </h3>
                      <h3 class="tracking-wide sm:text-md text-sm ">
                      ...
                      <button
                        disabled={
                          project
                            ? !project.accepting || isInTeam || applicationSent
                            : true
                        }
                        class={`${
                          project.accepting && !isInTeam && !applicationSent
                            ? "hover:bg-blue-700 bg-blue-600 hover:shadow-xl active:shadow-sm"
                            : "hover:bg-blue-300 bg-blue-300 hover:shadow-sm active:shadow-sm"
                        } shadow-sm sm:p-3 p-2 rounded-sm sm:text-md text-sm uppercase text-white sm:font-medium font-semibold tracking-wide sm:px-5 px-1 xl:mt-[87px] sm:mt-[75px] sm:top-0 top-5 mt-[60px] `}
                        onClick={() => setApplying(true)}
                      >
                        Apply - Send a Join Request
                      </button>

                      {isInTeam ? (
                        <div class=" sm:mt-[95px] mt-[77px] -mb-[95px] text-red-700 relative">
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            class="h-5 w-5 relative inline mr-2 bottom-[1px]"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                            stroke-width="2"
                      ...
                      {project ? (
                        applicationSent ? (
                          <div class=" sm:mt-[100px] mt-[85px] text-green-700 relative">
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              class="h-5 w-5 relative inline mr-2 bottom-[1px]"
                              viewBox="0 0 20 20"
                          ...
                          <div class=" sm:mt-[100px] mt-[80px] sm:top-0 sm:mb-0 mb-4 top-5 text-green-700 relative">
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              class="h-5 w-5 relative inline mr-2 bottom-[1px]"
                              viewBox="0 0 20 20"
            ...
            <div class="bg-white px-4 pt-2 pb-4 sm:p-6 sm:pb-4">
              ...
              <div class="mx-auto justify-center block sm:left-2 left-3 relative text-center">
                ...
                <button
                  ...
                  class="w-28 p-1.5 rounded-md font-medium tracking-wide shadow-md mt-3  bg-blue-400 hover:bg-blue-600  text-sm hover:shadow-xl active:shadow-md text-white  uppercase "
                >
                  Go Back
                </button>
              </div>
              ...
              <div class="sm:pb-6 top-1 sm:p-2 px-4 pb-6 shadow-md relative mx-auto block justify-center sm:left-2 left-3 bg-gradient-to-br from-blue-50 to-indigo-200 w-full sm:mt-4 mt-7  sm:mb-4 mb-8 rounded-md">
                <h4 class="text-gray-600 text-center sm:w-64 w-[203px] mx-auto relative top-2 text-sm">
                  Note: your profile details will be sent along with this
                  application
                </h4>
                ...
                <form onSubmit={submitHandler}>
                      ...
                      {project.application.map((question, index) => {
                        return (
                          <div key={index} class="relative mb-7">
                            <label
                              htmlFor="about"
                              className="block text-md mb-1 relative text-center font-medium text-gray-700"
                            >
                              {question}
                            </label>
                            <div className="mt-1">
                  ...
                  {!appLoading ? (
                    <button
                      type="submit"
                      class="bg-gradient-to-r mt-12 mb-2 tracking-wide hover:from-blue-500 hover:to-indigo-500 hover:shadow-xl active:shadow-sm from-blue-400 to-indigo-400 text-white uppercase text-lg px-16 py-2 rounded-md font-medium relative mx-auto block shadow-md"
                    >
                      Submit
                    </button>
                  ) : (
