  ...
  const removeProfPic = (e) => {
      ...
      <div class="mt-5 md:mt-0 md:col-span-3">
        <form id = 'regForm'>
          <div class="shadow overflow-hidden sm:rounded-md">
              <h1 class = 'text-blue-600 text-4xl mb-4 top-2 relative font-semibold text-center'>Edit <span class = 'text-black'>Your Profile</span></h1>
            <div class="px-3 py-2 bg-gradient-to-r from-blue-200 to-indigo-300 sm:p-6">
            ...
            <div className="relative scale-75 rounded-r-full p-3">
                    ...
                    {currentUser.user.profilePic? <button onClick = {(e)=> {e.preventDefault(); removeProfPic()}} class = 'font-semibold  p-2 shadow-md  z-20 bg-gray-100 hover:bg-gray-200 px-4 rounded-md hover:cursor-pointer hover:text-indigo-600 text-blue-600'>Remove Picture</button>:

                    <button onClick={(e)=>{e.preventDefault(); inputRef.current.click()}} class = 'font-semibold  p-2 shadow-md left-1  z-20 bg-gray-100 hover:bg-gray-200 px-4 rounded-md hover:cursor-pointer hover:text-indigo-600 text-blue-600'>Upload Picture</button>}
                   {currentUser.user.profilePic? <button class = 'font-semibold  p-2 shadow-md  z-20 bg-gray-100 hover:bg-gray-200 px-4 rounded-md hover:cursor-pointer hover:text-indigo-600 text-blue-600' onClick={(e)=>{e.preventDefault(); inputRef.current.click()}}>Change Picture</button>:''}
                    </ul>  
               </div>
   ...
   <option value="Zimbabwe">Zimbabwe</option>
                ...
                <div class="col-span-3">
                  ...
                  <input type="number" min={12}  value = {user?user.age:''}   onChange = {handleChange} name="age" placeholder={currentUser?currentUser.user.age:''} id="school"  class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2   shadow-md sm:text-sm border-gray-300 rounded-md"/>
                </div>
  
                <div class="col-span-6 sm:col-span-6 lg:col-span-3">
                  <label for="city" class="block text-sm font-semibold left-0.5 text-gray-700">City</label>
                  <input type="text"  value = {user?user.city:''}   onChange = {handleChange} placeholder={currentUser?currentUser.user.city:''} name="city" id="city"  class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2   shadow-md sm:text-sm border-gray-300 rounded-md"/>
                </div>
  
                <div class="col-span-6 sm:col-span-3 lg:col-span-3">
                  <label for="region" class="block text-sm font-semibold left-0.5 text-gray-700">Institution (School/University)</label>
                  <input type="text"   value = {user?user.school:''}  onChange = {handleChange} name="school" id="region" placeholder={currentUser?currentUser.user.school:''} min = {3} autocomplete="School" class="mt-1  focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2   shadow-md sm:text-sm text-black border-gray-300 rounded-md"/>
                </div>
                ...
                <div class="col-span-6 sm:col-span-8 lg:col-span-3">
                  ...
                  <input type="password"   onChange = {handleChange} name="password" id="city" autocomplete="password" placeholder="" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2   shadow-md sm:text-sm border-gray-300 rounded-md"/>
                </div>

                  <div class="col-span-6 sm:col-span-8  mb-3 lg:col-span-6">
                <label for="city" class="block text-sm font-semibold left-0.5 text-gray-700">Description</label>
                <textarea name = 'description' value = {user?user.description:''} placeholder={currentUser.user.description?currentUser.user.description:'Add Description - Your Background in STEM, Your Education, Summarize who you are'}     onChange={handleChange} class = 'mt-1 h-32  focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2  shadow-md sm:text-sm text-black border-gray-300 rounded-md'></textarea>
              </div>
