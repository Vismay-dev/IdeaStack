...
:
            ...
            {editing?
             ...
             <input  required name = 'title' onChange={handleChangeEdit} value = {editedInterest.title} class = 'mx-auto p-2 justify-self-center text-gray-700 relative shadow-lg text-sm  rounded-md mb-3'></input><br/>
             <textarea max = {120} name = 'desc' required onChange = {handleChangeEdit} value = {editedInterest.desc} class = '  md:w-9/12 sm:w-11/12 w-full text-gray-700 text-sm p-2 mx-auto relative justify-self-center shadow-lg h-24 rounded-md '></textarea>
             <br/>
              <button type = 'submit' class = 'md:w-1/4 w-2/4 font-semibold p-2 md:mt-2 mt-4 md:mb-6 mb-4 shadow-md mx-auto relative text-center z-20 bg-gray-100 hover:bg-gray-200  text-sm lg:text-md rounded-md hover:cursor-pointer hover:text-indigo-600 text-blue-600 mr-2'>Submit</button>
               <button onClick = {(e)=> {
                   e.preventDefault()
                   setTextArea(false)
                   setEditing(false)
               }} class = 'md:w-1/4 w-2/4 md:right-0 right-[3px] md:mt-0 -mt-6 md:mb-0 mb-6 font-semibold p-2 shadow-md mx-auto relative text-center z-20 bg-gray-100 hover:bg-gray-200   text-sm lg:text-md rounded-md hover:cursor-pointer hover:text-indigo-600 text-blue-600'>Cancel</button>
            <br/>
            </form>
            :
            ...
            <textArea max = {120} name = 'desc' required onChange = {handleChange} placeholder = 'Brief Description (Max 120 characters)' class = ' md:w-9/12 sm:w-11/12 w-full text-black text-sm p-2 mx-auto relative justify-self-center shadow-lg h-24 rounded-md '></textArea>
            <br/>
             <button type = 'submit' class = 'md:w-1/4 w-2/4 font-semibold p-2 md:mt-2 mt-4 md:mb-6 mb-4 shadow-md mx-auto relative text-center z-20 bg-gray-100 hover:bg-gray-200 text-sm lg:text-md rounded-md hover:cursor-pointer hover:text-indigo-600 text-blue-600 mr-2'>Submit</button>
              <button onClick = {(e)=> {
                  e.preventDefault()
                setTextArea(false)
              }} class = 'md:w-1/4 w-2/4 md:right-0 right-[3px] md:mt-0 -mt-6 md:mb-0 mb-5 font-semibold p-2 shadow-md mx-auto relative text-center z-20 bg-gray-100 hover:bg-gray-200 text-sm lg:text-md rounded-md hover:cursor-pointer hover:text-indigo-600 text-blue-600'>Cancel</button>
<br/>
</form>}
...
<div class = 'grid lg:grid-cols-3 md:grid-cols-4 grid-cols-3 space-x-3 mb-10 -mt-5'>
{currentUser.user.interests.map((interest,i)=> {
          return (
              <div class="max-w-xs lg:col-span-1 md:col-span-2 col-span-3 lg:col-start-1  md:col-start-2 col-start-1 mx-auto top-5 relative mb-5 overflow-hidden bg-gradient-to-r from-blue-50 to-indigo-100  rounded-lg shadow-lg ">
                <div class="px-4 py-2">
