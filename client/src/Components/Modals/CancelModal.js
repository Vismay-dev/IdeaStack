    ...
    <div class="max-w-md w-full space-y-8">
      ...
      <img class="mx-auto w-32 -mt-3 relative" src={logo} alt="Workflow"/>
        <h2 class="mt-1 bottom-3 relative mb-1 text-center sm:text-3xl text-2xl font-extrabold text-gray-900">
          Do you wish to cancel the latest booked mentorship session?
        </h2>
      </div>
        ...
        <div>
          <button onClick = {()=> {
              ...
              axios.post(process.env.NODE_ENV ==='production'?'https://ideastack.herokuapp.com/api/project/cancelLatestMentorship':'http://localhost:4000/api/project/cancelLatestMentorship',{token:sessionStorage.getItem('token'), projectID:sessionStorage.getItem('managing')}).then(res=> {
