...
const ViewProfile = () => {
  ...
  useEffect(() => {
    ...
    const [image, setImage] = useState(user&&user.profilePic?user.profilePic:null)
    ...
    useEffect(()=> {
        setLoading(true)
        axios.post(process.env.NODE_ENV ==='production'?"https://ideastack.herokuapp.com/api/user/getUserView"
        :"http://localhost:4000/api/user/getUserView",{token:localStorage.getItem('viewToken')}).then(res=> {
    ...
    useEffect(()=> {
      setLoading(true)
      axios.post(process.env.NODE_ENV ==='production'?"https://ideastack.herokuapp.com/api/user/getUserProjectsView"
      :"http://localhost:4000/api/user/getUserProjectsView",{token:localStorage.getItem('viewToken')}).then(res=> {
          ...
          <div  className="px-6">
            <div data-aos={"zoom-in-up"} data-aos-once='true'  className="flex flex-wrap justify-center">
              <div   className="lg:w-4/12 w-[270px] xl:w-3/12 xl:ml-10   px-4  lg:order-2 order-1 flex justify-center">
                <div className="relative scale-90 rounded-r-full  p-3">
                {
                  ...
                  <img class={`rounded-full -mt-16 -mb-2  w-56 h-56 object-contain bg-white  shadow-lg block ${user&&user.profilePic?'':'p-2'} relative`}
                src={user&&user.profilePic?user.profilePic:"https://media.istockphoto.com/vectors/default-profile-picture-avatar-photo-placeholder-vector-illustration-vector-id1223671392?k=20&m=1223671392&s=612x612&w=0&h=lGpj2vWAI3WUT1JeJWm1PRoHT3V15_1pdcTn2szdwQ0="} /> }
                ...
                <div className="flex justify-center py-4 xl:right-7 lg:right-3 right-0 relative lg:pt-4 pt-8">
                  <div className="mr-4 p-3 text-center">
                    <span className="text-xl font-bold block uppercase tracking-wide text-blueGray-600">
                      {user&&user.projects.filter(onlyUnique).length}
                    </span>
            ...
            <div  className="text-center mt-11">
              ...
              <div className="mb-2 text-blueGray-600 px-4">
                ...
                <span class = 'sm:font-normal font-medium'>Age:</span> {user?user.age:' '}
              </div>
              <div className="mb-2 text-blueGray-600 px-4">
              ...
              <div className="sm:mb-5 mb-8 text-blueGray-600 px-4">
                ...
                <span class = 'sm:font-normal font-medium'>Contact</span>: {user?user.email:' '}
              </div>
             ...
             <div className="mt-12 py-10 border-y border-gray-200 text-center">
              <div className="flex flex-wrap justify-center">
                <div className="w-full lg:w-9/12 px-4">
                   ...
                   <p style={{whiteSpace: "pre-wrap"}}>{user.description}</p>  
                    ...
                    <>
                     <i className="fas fa-exclamation text-indigo-600 mb-2 text-4xl "></i>
                        <p class = 'text-gray-800 font-semibold text-lg mb-5'>No Description Available</p>
                    </>
                  }
                  </p>
                  ...
                  <button
                    class={`font-semibold top-3  ${!showmore?'z-[75]':'z-30'} p-2 text-md shadow-md mb-2 bg-gray-100 hover:bg-gray-200 px-4 rounded-md hover:cursor-pointer hover:text-indigo-600 relative text-gray-700`}
                    onClick={() => {
            ...
            
  {
    ...
    {user.interests.map((interest,i) => {
      return (
          <div class={`w-full relative  mx-auto py-2 pb-1 ${user.interests.length===1?'md:col-start-2 sm:col-start-2':''} md:col-span-1 sm:col-span-2 lg:left-0 left-[1px] col-span-3 top-5 relative mb-5 overflow-hidden bg-gradient-to-r from-blue-50 to-indigo-100  rounded-lg shadow-lg `}>
 ...
 <p class = 'text-gray-800 font-semibold text-lg mb-16'>No Interests Available</p>
 </div>
</>}                          
      ...
      return(
        ...
        <div class={`rounded-lg shadow-lg bg-gradient-to-r  from-blue-50 to-indigo-100 sm:w-full w-[120%] sm:right-0 right-[10%] relative  overflow-hidden mb-10`}>
           <img
              src={proj.projPic}
              alt="image"
              class="w-full h-48 object-contain py-3 -mb-3 bg-gray-50 border-b-2 border-gray-400 relative"
              />
           <div class="p-8 sm:p-9 md:p-7 xl:p-9 text-center">
              ...
              <p class="text-base text-body-color mt-4 leading-relaxed mb-2">
              <strong class = 'underline'>Category:</strong> {proj.category}
              </p>
              <p class="text-base text-body-color mt-1 leading-relaxed">
              <strong class = 'underline'>Number of Members:</strong> {proj.team.length}
              </p>
